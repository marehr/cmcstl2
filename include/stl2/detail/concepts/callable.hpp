...
STL2_OPEN_NAMESPACE {
	...
	namespace ext {
		...
		STL2_CONCEPT_KEYWORD IndirectInvocable =
			(Readable<Is> && ... && true) &&
			CopyConstructible<F> &&
	...
	STL2_CONCEPT_KEYWORD IndirectUnaryInvocable =
	...
	namespace ext {
		template <class F, class... Is>
		STL2_CONCEPT_KEYWORD IndirectRegularInvocable =
			IndirectInvocable<F, Is...>;
	}

	template <class F, class I>
	STL2_CONCEPT_KEYWORD IndirectRegularUnaryInvocable =
	...
	namespace ext {
		template <class F, class... Is>
		STL2_CONCEPT_KEYWORD IndirectPredicate =
			(Readable<Is> && ... && true) &&
			CopyConstructible<F> &&
			// The following 3 are checked redundantly, but are called out
			// specifically for better error messages on concept check failure.
	...
	template <class F, class I>
	STL2_CONCEPT_KEYWORD IndirectUnaryPredicate =
		ext::IndirectPredicate<F, I>;

	template <class F, class I1, class I2 = I1>
	STL2_CONCEPT_KEYWORD IndirectRelation =
		Readable<I1> &&
		Readable<I2> &&
		CopyConstructible<F> &&
	...
	template <class F, class I1, class I2 = I1>
	STL2_CONCEPT_KEYWORD IndirectStrictWeakOrder =
		Readable<I1> &&
		Readable<I2> &&
		CopyConstructible<F> &&
