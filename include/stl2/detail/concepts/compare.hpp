...
STL2_OPEN_NAMESPACE {
	...
	STL2_CONCEPT_KEYWORD WeaklyEqualityComparable =
	...
	//
	template <class T>
	STL2_CONCEPT_KEYWORD EqualityComparable =
		WeaklyEqualityComparable<T, T>;

	template <class T, class U>
	STL2_CONCEPT_KEYWORD EqualityComparableWith =
	...
	//
	template <class T, class U>
	STL2_CONCEPT_KEYWORD __totally_ordered =
		requires(const remove_reference_t<T>& t,
		         const remove_reference_t<U>& u) {
			{ t <  u } -> Boolean&&;
			{ t >  u } -> Boolean&&;
