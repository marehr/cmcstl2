...
STL2_OPEN_NAMESPACE {
	...
	namespace __begin {
		...
		template <class R>
		STL2_CONCEPT_KEYWORD has_member = _Is<R, std::is_lvalue_reference> && requires(R r) {
			r.begin();
			{ __decay_copy(r.begin()) } -> Iterator;
		};

		template <class R>
		STL2_CONCEPT_KEYWORD has_non_member = requires(R&& r) {
	...
	namespace __rbegin {
		...
		template <class R>
		STL2_CONCEPT_KEYWORD can_make_reverse = requires(R&& r) {
			typename __begin_t<R>;
			typename __end_t<R>;
	...
	namespace __size {
		...
		template <class R>
		STL2_CONCEPT_KEYWORD has_member = !disable_sized_range<__uncvref<R>> &&
			requires(R&& r) {
		...
		template <class R>
		STL2_CONCEPT_KEYWORD has_non_member = !disable_sized_range<__uncvref<R>> &&
			requires(R&& r) {
		...
		template <class R>
		STL2_CONCEPT_KEYWORD has_difference = requires {
			typename __begin_t<R>;
			typename __end_t<R>;
		} &&
	...
	namespace __empty {
		template <class R>
		STL2_CONCEPT_KEYWORD has_member = requires(R&& r) {
			static_cast<R&&>(r).empty();
			static_cast<bool>(static_cast<R&&>(r).empty());
		};

		template <class R>
		STL2_CONCEPT_KEYWORD has_size = requires(R&& r) {
			__stl2::size(static_cast<R&&>(r));
		};

		template <class R>
		STL2_CONCEPT_KEYWORD has_begin_end = requires {
			typename __begin_t<R>;
			typename __end_t<R>;
	...
	namespace __data {
		...
		template <class R>
		STL2_CONCEPT_KEYWORD has_pointer_iterator = requires {
			typename __begin_t<R>;
			requires is_object_ptr<__begin_t<R>>;
		};

		template <class R>
		STL2_CONCEPT_KEYWORD has_contiguous_iterator = requires {
			typename __begin_t<R>;
			typename __end_t<R>;
